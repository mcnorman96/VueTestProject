...
<template>
  <PageLayout>
    <template v-slot:title> Single User </template>
    <template v-slot:pageContent>
      ...
      <div v-if="user" class="max-w-2xl overflow-hidden bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
          <h3 class="text-lg font-medium leading-6 text-gray-900">User: {{ user.username }}</h3>
          <p class="max-w-2xl mt-1 text-sm text-gray-500">Details and informations about user.</p>
        </div>
